...
export default function Nav() {
  ...
  return (
    <>
      <nav className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 w-[95%] sm:w-[90%] max-w-5xl z-50 rounded-full bg-white/60 backdrop-blur-md shadow-soft px-3 sm:px-6 py-2.5 sm:py-4 flex items-center justify-between transition-all duration-300 border border-slate-200/60">
        <Link href="/" className="font-serif italic font-medium tracking-tight text-xl sm:text-2xl text-[#1a1a1a] ml-1 sm:ml-2">
          Mnemosyne
        </Link>
        <div className="flex items-center gap-1 sm:gap-3">
          <div className="flex gap-1 sm:gap-2 mr-2 sm:mr-6">
            {navLinks.map((link) => {
              ...
              return (
                <Link 
                  key={link.href} 
                  href={link.href} 
                  className={`relative text-xs sm:text-sm font-bold px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-full transition-all flex items-center justify-center ${
                    isActive 
          ...
          <Link href="/contact" className="flex items-center gap-1 sm:gap-2 bg-[#f8a8b8] hover:bg-[#f292a5] text-slate-900 px-3 sm:px-6 py-1.5 sm:py-2.5 rounded-full font-bold transition-all shadow-sm">
          ...
          <div className="relative ml-2">
            <button
              ...
              className={`flex items-center gap-2 sm:gap-3 px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-full border border-slate-200/80 transition-all shadow-sm bg-white/60 backdrop-blur-md hover:bg-white/90 hover:shadow-md ${aiStatus === "rate_limited" ? "animate-pulse" : ""}`}
            >
